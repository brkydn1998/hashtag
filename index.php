...
function getserver(){
    return  "http://olcayhizal.com.tr/msj/";
...
function gethashtag($count){
    $server = getserver();
    $array = getresult($server."api.php?gethashtag=$count");
    return $array;
}
...
function tara($count){
    $urllist = array();
    $hashtagler = gethashtag($count);
    $tumsonuc = array();

    foreach ($hashtagler as $key => $hashtag) {
        $servers = servers();
    ...
    foreach ($responses as $reskey => $result) {
        ...
        for($i=0;$i<$esayi;$i++){
            ...
            $aciklama = remove_emoji($aciklama);

            $aciklama = str_replace("'", "", $aciklama);
            $sonuc['aciklama'] = $aciklama;
            $hashtagx = $hashtagler[$reskey];
            $sonuc['hashtag'] = $hashtagx;
            ...
            if($numara != "NULL"){
                if(!in_array($numara, $cikanlar, true)){
                    array_push($cikanlar, $numara);
                    $sonuc['numara'] = $numara;
                }
            }

            array_push($tumsonuc,$sonuc);
...
$vowels = array("<", ">", '"', "'", "%", ";", "(", ")", "&", "+");
if(isset($_GET['count'])) {$count = str_replace($vowels, "", $_GET['count']);}else{$count="";}
